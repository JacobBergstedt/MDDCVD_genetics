...
CAD_trait_fit_out="Results/Mixer/CAD_2022/CAD_2022_vs_""${input}"".fit.${REP}"
CAD_trait_param="${mdd_trait_fit_out}.json"
CAD_trait_test_out="Results/Mixer/CAD_2022/CAD_2022_vs_""${input}"".test.${REP}"

# ${PYTHON} /tools/mixer/precimed/mixer.py fit1 ${MIXER_COMMON_ARGS} ${EXTRACT} --trait1-file ${trait_path} --out ${trait_fit_out}
${PYTHON} /tools/mixer/precimed/mixer.py fit2 ${MIXER_COMMON_ARGS} ${EXTRACT} --trait1-file ${CAD_path} --trait2-file ${trait_path} --trait1-params ${CAD_param} --trait2-params ${trait_param} --out "${CAD_trait_fit_out}"
...
${PYTHON} /tools/mixer/precimed/mixer.py test2 ${MIXER_COMMON_ARGS} --trait1-file ${CAD_path} --trait2-file ${trait_path} --load-params ${CAD_trait_param} --out ${CAD_trait_test_out}
