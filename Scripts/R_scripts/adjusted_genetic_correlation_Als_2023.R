...
type_models <- keys_types %>% filter(!Trait %in% c("PP", "SBP", "DBP", "CM_2021_V2")) %>% 
  group_by(Type) %>% 
  summarize(Model = paste0(Trait, collapse = " + ")) %>% 
  ungroup() %>% 
  pull(Model)

gen_cov <- readRDS("Data/Genetic_covariance_matrices/genetic_covariance_Als_2023.rds")

grid <- expand.grid(trait1 = "MDD_Als_2023", trait2 = cvd_traits, adjustment = c("Nothing", get_MDD_var_list_without_CVD(), type_models), stringsAsFactors = FALSE)
